...
const mapAuthErrorToSpanish = (errCode: string): string => {
  switch (errCode) {
    case 'auth/email-already-in-use':
      return '¡Este correo electrónico ya está registrado! Por favor, inicia sesión con tu cuenta.';
    case 'auth/invalid-email':
      return 'El correo electrónico ingresado no es válido.';
    case 'auth/weak-password':
      return 'La contraseña es muy débil. Debe tener al menos 6 caracteres.';
    case 'auth/operation-not-allowed':
      return 'El registro por correo/contraseña no está habilitado. Por favor, asegúrate de activar "Correo electrónico/contraseña" en tu consola de Firebase.';
    case 'auth/network-request-failed':
      return 'Error de red. Verifica tu conexión a internet.';
    case 'auth/popup-closed-by-user':
      return 'Se cerró la ventana de inicio de sesión de Google antes de completarse.';
    default:
      return 'Ocurrió un error inesperado al registrar el usuario. Por favor intenta de nuevo.';
  }
};
    ...
    <div className="min-h-screen flex flex-col items-center justify-center bg-[#0A0E17] p-4 text-white">
      <div className="w-full max-w-md bg-[#0F1420] rounded-[32px] shadow-2xl border border-slate-800 p-8 relative overflow-hidden">
        <div className="flex flex-col items-center mb-8">
          ...
          <h1 className="text-3xl font-bold tracking-tight text-white uppercase italic">Crear Cuenta</h1>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div>
            <label className="block text-[10px] uppercase tracking-widest font-bold text-slate-500 mb-2">Nombre para mostrar</label>
            <input
              type="text"
              value={displayName}
        ...
        <p className="mt-8 text-center text-xs font-bold uppercase tracking-widest text-slate-500">
          ¿Ya tienes una cuenta? <Link to="/login" className="text-blue-400 hover:text-white transition-colors">Inicia sesión</Link>
        </p>
      </div>
    </div>
  );
